...
- **The beat commit (fast path)**: `commit_beat(game, resume, log, edits, appends, writes, promises, threads, units)` - the whole beat's persistence in ONE call, engine-applied with the individual tools' guards, per-item results, sealed on success. The default way to persist a beat; fix any FAIL items before speaking. Promises, threads, and units are first-class cargo: the engine owns all three grammars (unit cargo edits strength/tags/loyalty in place - casualties are ledger math). (Older cached tool list: the individual tools plus `end_beat`, same thing, slower.)
