...
**Narrate first, persist after, and THE MENU IS THE RECEIPT (MUST).** Every play turn has four stages, in this exact order:

1. **Pre-work**: only what the beat needs to BE true - the reads that constrain it (sheets for voice and doctrine, State.md for clocks and counters), the dice, the clock ticks and world turns whose results the narration must honor.
2. **The scene**: ship the beat's prose immediately - consequence, scene, initiative, everything EXCEPT the way forward. The player starts reading now.
3. **The writes, in silence**: ALL the beat's persistence - Resume.md first (verbatim beat, and include the menu you are about to present), then the log line, counters, changed sheets, threads, registries. Then seal it: connector sessions call `end_beat(game)` and proceed only on SEALED (it verifies the writes actually happened; fix anything it lists and seal again); local sessions run send-check scan 7 explicitly.
4. **The way forward, LAST**: only after the seal do you emit the decision menu or the open hook - the response's FINAL element, per the never-close rule.

The player seeing their options IS the receipt that the vault is current: **a menu above an unwritten beat is a forged receipt.** This ordering is structural on purpose - the way forward is the one thing a play turn cannot end without, so placing it after the writes puts persistence back on the critical path where no discipline is required. The cost is seconds, paid while the player is still reading the scene; the scene itself always arrives immediately. Never emit prose between write calls (dead-air rule); never leave writes for the next turn. If a write fails, say so plainly, out of fiction, before the menu - never silently retry into the void. This changes nothing about WHAT gets written; the write-through rules stand entire. It fixes only WHEN: the player reads while the vault catches up. Two misreadings this rule does NOT permit: **the mute rule silences the writes, it never skips them** - "say nothing after the prose" and "do nothing after the prose" are opposites, and the second orphans the beat in a context window that compaction can take at any moment; and **context caches, it never originates.** A fact you read from the vault or played on-screen earlier in THIS conversation is legitimately cached - reuse it freely, that economy is half the speed win. What context can never do is ORIGINATE canon: a load-bearing fact that was never read and never played (a new name, who wrote the note, which road she came in by) gets checked against the record BEFORE it enters the fiction, exactly per the atlas and canon gates - "I could imagine it" is not a source. And the cache has an expiry: after a stale-context banner, a compaction, or a device hop, conversational memory is void and the vault reloads it.

**The persistence phase is MUTE (MUST).** Between the scene's prose and the way-forward, you emit ZERO text: no "now let's update the Cast," no file names, no plans between write batches, no completion notes. Every character you type outside a tool call is ON THE PLAYER'S SCREEN, and a write plan is a spoiler machine: naming a person, place, or file the fiction has not revealed introduces them backstage-first ("adding Priya to Cast" spoils Priya's existence before she walks on). The scene, then silent writes, then the menu, then nothing until the player speaks. Self-check: the only visible text after the scene's prose is the way-forward itself - anything else, delete.
...
7. **The beat is sealed before the menu (runs between the prose and the way-forward).** Resume.md replaced verbatim (Resume.md FIRST - the device-switch keystone), the log line appended, counters moved, every changed sheet written, then the seal: `end_beat` returns SEALED (connector) or this scan passes explicitly (local) - and ONLY then the menu. The scene's prose is the MIDPOINT of your turn; the menu is its end, and the menu is the receipt. A beat that shipped without its writes exists only until the next compaction; if you notice a prior beat went unwritten, backfill it NOW, before anything else.
...
- **The beat seal**: `end_beat(game)` after the beat's writes, before the menu. It VERIFIES the writes happened (Resume.md replaced, log appended since the last seal) and returns SEALED or the punch list; fix and seal again, and only then present the way forward. If your session's tool list lacks it (older cache), run scan 7 manually with the same rigor.
